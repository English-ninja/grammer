...
function loadStateFromLocalStorage() {
  const savedState = localStorage.getItem('english_mastery_state');
  if (savedState) {
    try {
      const parsed = JSON.parse(savedState);
      state.completedDays = parsed.completedDays || {};
      state.confidence = parsed.confidence || {};
      state.homework = parsed.homework || {};
      state.subtopicsChecked = parsed.subtopicsChecked || {};
...
function saveStateToLocalStorage() {
  const dataToSave = {
    ...
    subtopicsChecked: state.subtopicsChecked,
    completedDates: state.completedDates,
    streak: state.streak
  };
  localStorage.setItem('english_mastery_state', JSON.stringify(dataToSave));
}
